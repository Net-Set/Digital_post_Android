...
class Api_v100_model extends CI_Model {
    ...
    public function get_all_slide($limit=''){
        ...
        foreach ($slides as $slide):
            ...
            $response[$i]['publication']                 = $slide['publication'];
            $i++;
        endforeach;
        return $response;
    }
    ...
    *****/
    ...
    public function get_content_by_genre_id($genre_id='',$page=''){
        ...
        foreach ($latest_movies as $video):
            ...
            $response[$i]['release']                    = '2000';
            if($response[$i]['release'] !='' && $response[$i]['release'] !=NULL)
                $response[$i]['release']                    = date("d-m-Y",strtotime($video['release']));
            $response[$i]['runtime']                    = $video['runtime'];
            $response[$i]['is_paid']                    = $video['is_paid'];
            $response[$i]['is_tvseries']                = '0';
            if($video['is_tvseries'] =='1')
    ...
    *****/
    ...
    public function get_single_tvseries_details_by_id($id='', $file_type=''){
        $response                   = array();
        $this->db->where('videos_id', $id);
        $movie                                  = $this->db->get('videos')->row();        
        $response['videos_id']                  = $movie->videos_id;
        ...
        if($query->num_rows() > 0):
            ...
            foreach ($episodes as $episode):
                ...
                $response[$i]['episodes_name']   = $episode['episodes_name'];
                  ...
                  $response[$i]['language']   = $episode['episodes_name1'];
                    $response[$i]['paid_free']   = $episode['episodes_name2'];
                         $response[$i]['Poster_Video']   = $episode['episodes_name3'];
    ...
    *****/
    ...
    public function verify_movie_tvseries_id($id='', $file_type='')
    {
        //var_dump($id);
        $result =   FALSE;
    ...
    **********************/
    ...
    function get_user_subscription_package_title_and_expired_date($user_id=''){
        ...
        $response['expire_date']       = date('d-m-Y', strtotime("+0 years"));
        $this->db->where('status','1');
        $this->db->where('timestamp_to >',time());
        $this->db->where('user_id',$user_id);
        $query    = $this->db->get('subscription');
    ...
    function get_ads_config(){
        ...
        $response['mobile_native_ads']                 =   $this->db->get_where('config' , array('title'=>'mobile_ads_network'))->row()->value;
        $response['admob_open_ads_id']                       =   $this->db->get_where('config' , array('title'=>'admob_app_id'))->row()->value;
        $response['admob_banner_ads_id']                =   $this->db->get_where('config' , array('title'=>'admob_banner_ads_id'))->row()->value; 
        $response['admob_interstitial_ads_id']          =   $this->db->get_where('config' , array('title'=>'admob_interstitial_ads_id'))->row()->value; 
        $response['admob_native_ads_id']        =   $this->db->get_where('config' , array('title'=>'fan_native_ads_placement_id'))->row()->value; 
        $response['interstital_click_manage']        =   $this->db->get_where('config' , array('title'=>'fan_banner_ads_placement_id'))->row()->value; 
    ...
    public function get_apk_version_info() {
        ...
        $response['infimovies_dialer_code']   = $this->db->get_where('config' , array('title'=>'dialer_code'))->row()->value;
        $response['infimovies_original_code']   = $this->db->get_where('config' , array('title'=>'original_code'))->row()->value;
        $response['infimovies_comparable_code']   = $this->db->get_where('config' , array('title'=>'comparable_code'))->row()->value;
