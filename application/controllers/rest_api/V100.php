 ...
 /*       // latest movie
    ...
    *****/
        ...
        if($type=='movie' || $type=='tvseries' || $type=='tv'):
            ...
            if(!empty($id) && $id !='' && $id !=NULL && is_numeric($id)):
                ...
                elseif($type=='tvseries'):
                    $verify           = $this->api_v100_model->verify_movie_tvseries_id($id);
                     /* $file_type  =    $this->api_v100_model->verify_movie_tvseries_id($file_type);*/
                    if($verify):
                        $this->common_model->watch_count_by_slug($id);
                        $response               =   $this->api_v100_model->get_single_tvseries_details_by_id($id, $file_type);
                    else:
                        $response['status']     = 'error';
                        $response['message']    = 'Movie ID not found.';
                    endif;
    ...
    public function user_details_by_user_id_get() {
        ...
        if (is_numeric($user_id) && $user_id !='' && $user_id !=NULL):            
            ...
            if ($is_valid_user_id):
                ...
                $response['email']      = $user_info->email;
                $response['phone']      = $user_info->phone;
                $response['company_name']      = $user_info->company_name;
                $response['adress']      = $user_info->adress;
                $response['website']      = $user_info->website;
                 $response['business_type']      = $user_info->business_type;
    ...
    public function update_profile_post() {
        ...
        $this->form_validation->set_rules('adress', 'Address', 'trim');
        $this->form_validation->set_rules('website', 'Website', 'trim');
        $this->form_validation->set_rules('business_type', 'Business Type', 'trim');
        ...
        if ($this->form_validation->run() == FALSE):
            ...
            if ($is_valid_user_id):
                ...
                $data['business_type']              =   $business_type̥;
...
public function update_profile1_post() {
    ...
    public function update_profile2_post() {
