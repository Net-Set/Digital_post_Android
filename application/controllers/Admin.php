...
class Admin extends CI_Controller {   
    ...
    function videos($param1 = '', $param2 = ''){
        ...
        $data['page_title']     = 'Sub-category Management';             
        $this->load->view('admin/index', $data);
    }
    ...
    function sub_category_add(){
        ...
        $data['page_title']     = 'Sub-category Add'; 
        $this->load->view('admin/index', $data);
    }
    ...
    function tvseries($param1 = '', $param2 = ''){
        ...
        if ($param1 == 'add') {
            ...
            $this->session->set_flashdata('success', 'Sub category added successed');
    ...
    function seasons_manage($param1='',$param2=''){
        ...
        $this->session->set_flashdata('success', 'years added successed');
        redirect($this->agent->referrer());
        }
        if ($param1 == 'update') {           
            ...
            $this->session->set_flashdata('success', 'years update successed.');
            redirect($this->agent->referrer());
        }
    ...
    function episodes_manage($param1='',$param2=''){
        ...
        elseif ($param1 == 'edit'):
            ...
            $data['page_title']     = "Poster For".' '.$this->common_model->get_title_by_videos_id($param1).' '.$this->common_model->get__seasons_name_by_id($param2);
            $this->load->view('admin/index', $data);
        endif;
    }
    ...
    function video_quality($param1 = '', $param2 = ''){
        ...
        $data['page_title']     = 'Poster quality Management';
    ...
    function send_movie_tvseries_notification($param1 = '', $param2 = ''){
        ...
        $data['page_title']     = 'Send poster Notification(OneSignal)';
        $this->load->view('admin/index', $data);
    }
    ...
    public function video_upload(){
     ...
     public function episodes_upload(){
        ...
        $episodes_name1              = $this->input->post('episodes_name1');
        $episodes_name2              = $this->input->post('episodes_name2');
        $episodes_name3              = $this->input->post('episodes_name3');
        $order                      = $this->input->post('order');
        $source                     = $this->input->post('source');
        // validation rule
        $this->form_validation->set_rules('videos_id', trans('video_id'), 'trim|required');
        $this->form_validation->set_rules('seasons_id', trans('seasons_id'), 'trim|required');
        $this->form_validation->set_rules('episodes_name3', trans('episode_name3'), 'trim|required');
        $this->form_validation->set_rules('episodes_name1', trans('episode_name1'), 'trim|required');
        $this->form_validation->set_rules('episodes_name2', trans('episode_name2'), 'trim|required');
        $this->form_validation->set_rules('source', trans('source'), 'trim|required');
        ...
        $data['episodes_name1']  = $episodes_name1;
        $data['episodes_name2']  = $episodes_name2;
        $data['episodes_name3']  = $episodes_name3;
        $data['order']          = $order;
        ...
        if($episodes_name3 == 'Video'):
            ...
            if($source == 'upload'):
                if($this->form_validation->run() == FALSE):
                    ...
                    if($this->upload->do_upload('videofile')):
                        ...
                        $this->session->set_flashdata('success', "Video added succesfully");
                    else:
                        $this->session->set_flashdata('error', $this->upload->display_errors());
            ...
            $this->session->set_flashdata('success', "Poster added succesfully");
        endif;
        redirect($this->agent->referrer());
    }
    public function episodes_update($episodes_id=""){
        ...
        if($episodes_name3 == 'Video'):
                    if(!empty($_FILES['videofile'])):
                        ...
                        if($this->upload->do_upload('videofile')):
                            ...
                            $data['source_type']    = 'upload';                    
                            $data['file_url']       = base_url().'uploads/videos/'.$file_name;
                            // $insert_id              = $this->common_model->get_episode_image_url($videos_id,$episodes_id).'?'.time();
                            // $insert_status          = TRUE;
                            // if (!empty($_FILES['thumbnail2']['name'])):                
                            //     $image_name                 = $insert_id.'.jpg';
                            //     $config['upload_path']      = 'uploads/episodes/';
        ...
        if($episodes_name3 == 'Poster'):
            ...
            if (!empty($_FILES['thumbnail']['name'])):                
                ...
                $this->session->set_flashdata('success', "Poster updated succesfully");
            endif;
            $this->session->set_flashdata('success', "Poster updated succesfully");
        endif;
        redirect($this->agent->referrer());
